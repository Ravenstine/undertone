...
class Demodulator extends Transform {
  ...
  _transform(chunk, encoding, next){
    ...
    const numBits  = bitStream.index || 0,
          numBytes = numBits / 8;
    if(numBytes <= 0 || !isInteger(numBytes)) return next();
    const newIndex = bitStream.index;
    // bitStream.index = this.cursor;
    bitStream.index = 0;
    const output = bitStream.readArrayBuffer(numBytes).slice(0, numBytes);
    // bitStream.index = newIndex;
    // this.cursor     = newIndex;
    bitStream.index = 0;
    // new Uint1Array(output).forEach(x => process.stdout.write(`${x}`));
