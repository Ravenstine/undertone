...
class Packet {
  ...
  static from(input, options){
    ...
    if(length > MAX_PACKET_DATA_SIZE)
      throw new Error(`Data exceeds maximum possible packet size of ${MAX_PACKET_DATA_SIZE} bytes.`);
  ...
  /**
   * @param {object} options
   * @param {ArrayBuffer=}  options.preamble - A byte pattern to signify the start of the packet.  Defaults to 24 alternating bits.
